...
class EventListener implements View.OnKeyListener,
    ...
    public void onClick(View view) {
        ...
        String str = ((ColorButton)view).getText().toString();
        ...
        case R.id.equal:
            ...
            Log.e("error",result);
            String[] soundToPlay = new String[result.length()];
            Log.e("error",result.length()+"");
            for(int i =0 ;i < soundToPlay.length;i++){
                soundToPlay[i] = String.valueOf(result.charAt(i));
            }
            Log.e("error",soundToPlay[0]+"  "+soundToPlay[1]);
            SoundManager.getInstance().playSeqSounds(soundToPlay);
            break;
